...
No jogo Pong, o jogador deve rebater uma bola com a sua plataforma a fim de faze-la alcançar o outro lado. Toda vez que a bola consegue atingir seu objetivo, o jogador recebe 1 ponto. O jogo termina quando 10 pontos são atingidos ou quando a bola passa pela plataforma, fazendo com que o jogador perca.

É possivel acessar o jogo pelo link: https://anderson-couto.github.io/cg-2d-game/public/index.html
...
- `gamedata.hpp`: Estrutura responsável por guardar o estado da aplicação;
- `window.hpp`: Classe com métodos e atributos para renderizar o jogo;
- `window.cpp`: Implementação dos métodos e atributos da classe Window. É aqui que são invocados os shaders, renderização de cores e atribuições de regras de negócio que atendem todo o jogo;
- `ball.hpp`: Classe com métodos e atributos relacionados a bolinha;
- `ball.cpp`: Implementação dos métodos e atributos da classe Ball;
- `paddle.hpp`: Classe com métodos e atributos relacionados a plataforma;
- `paddle.cpp`: Implementação dos métodos e atributos da classe Paddle;
- `assets\Inconsolata-Medium.ttf`– Fonte base utilizada para renderizar mensagens;
- `assets\objects.frag`– _Fragment shader_, é o objeto que possui as propriedades da bolinha, dos blocos e da plataforma, programa como os pixels são exibidos na tela, onde a cor de entrada é a mesma cor da saída.
- `assets\objects.vert`– _Vertex shader_, é o objeto que possui as propriedades de vértices da bolinha, dos blocos e da plataforma.
